"""Run GCNDIADTI on the Yamanishi enzyme subset.

Place this script in data/dataset/Yamanishi/enzyme and run:

python run_enzyme_gcndiadti.py

The script combines the parameter setup, preprocessing, and training steps for
the enzyme subset. It imports the main GCNDIADTI model files from the project
code directory and injects enzyme-specific dimensions before importing them.
"""
...
def parse_cli():
    ...
    parser = argparse.ArgumentParser(description="GCNDIADTI Yamanishi enzyme runner")
    ...
    parser.add_argument("--drug-num", default=445, type=int)
    parser.add_argument("--protein-num", default=664, type=int)
...
def preprocess_repeats(cli_args, model_args, data_dir, device):
    ...
    print("Loaded enzyme adjacency matrix as drug x protein: %s" % (tuple(dp.shape),))
...
def main():
    ...
    print("Enzyme dimensions: drug_num=%d, protein_num=%d, latdim=%d" %
